...
function App() {
  const { settings, is_loading, SaveSettings, settings_ref } =
    UseChromeStorage();
  UseTheme(settings.theme);

  const HandleThemeChange = useCallback(
    (theme: Theme) => {
      SaveSettings({ ...settings_ref.current, theme });
    },
    [SaveSettings, settings_ref]
  );

  const HandleEnabledChange = useCallback(
    (is_enabled: boolean) => {
      SaveSettings({ ...settings_ref.current, is_enabled });
    },
    [SaveSettings, settings_ref]
  );
